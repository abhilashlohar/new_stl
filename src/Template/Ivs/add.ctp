	...
	function rename_rows(){
		...
		$(".main_table tbody.main_tbody tr.main_tr").each(function(){
			...
			$(this).find('table.consumption_table tbody.consumption_tbody tr.consumption_tr').each(function(){
				...
				if(select_count>0){
					$(this).find('td:nth-child(3) select').select2().attr({name:"iv_left_rows["+invoice_row_id+"][iv_right_rows]["+q+"]['iv_right_serial_numbers']["+i+"][item_serial_number_id]", id:"iv_right_rows-"+i+"-serial_numbers"});
				}
			 i++; });
		q++; });
	}
